...
public class ExpenseHistoryDAO {

    private TrackingHistoryHelper mHelper;

    public ExpenseHistoryDAO(Context context) {
        this.mHelper = TrackingHistoryHelper.getInstance(context);
    }

    /**
     * 경비 테이블에 존재하는 모든 데이터를 로드합니다.
     */
    public Cursor findAll() {
        ...
        c = db.query(ExpenseHistory.class.getSimpleName(), null, null, null, null, null, null);

        String query = c.toString();
        Log.d("queryCheck : ", query);

        Log.d("size 확인확인", String.valueOf(c.getCount()));
    ...
    public boolean save(ExpenseHistory data) {
        ...
        values.put("trackingHistory_id", data.getTrackingHistory_id());
        values.put("expenseTitle", data.getExpenseTitle());
        values.put("expenseType", data.getExpenseType());
        ...
        long insertedId = db.insert(ExpenseHistory.class.getSimpleName(), null, values);

        return insertedId != -1;
    }

    public Cursor findExpenseHistoryAll() {
         ...
         * Cursor 객체는 해당 쿼리의 결과가 담기는 객체입니다.
         */
        return db.query(ExpenseHistory.class.getSimpleName(), null, null, null, null, null, "date" + " DESC");
    }
